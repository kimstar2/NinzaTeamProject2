...
namespace _TevLib.Extension.DoT
{
    ...
    public class TransformTweenSequencer : MonoBehaviour
    {
        ...
        [SerializeField] private List<TransformTweenStep> sequenceStep;

        [Header("Tween Options")] [SerializeField]
        ...
        public void SetSteps(List<TransformTweenStep> steps) => sequenceStep = steps;
        ...
        public bool SequenceAndResult()
        {
            ...
            foreach (TransformTweenStep step in sequenceStep)
        ...
        private bool AddStep(Sequence sequence, TransformTweenStep step)
        {
        ...
        private Tween MakeTween(TransformTweenStep step)
        {
        ...
        private Tween CreateMoveTween(TransformTweenStep step)
            => (IsCanvas ? _rectTrm : _transform).DOMove(step.GetTransformValue(), step.Duration);

        private Tween CreateLocalScaleTween(TransformTweenStep step)
            => (IsCanvas ? _rectTrm : _transform).DOScale(step.GetTransformValue(), step.Duration);

        private Tween CreateCanvasAlpha(TransformTweenStep step)
        {
        ...
        private Tween CreateAnchoredPositionTween(TransformTweenStep step)
        {
            if (!IsCanvas)
            {
        ...
        private Tween CreateLocalRotationTween(TransformTweenStep step)
            => (IsCanvas ? _rectTrm : _transform).DOLocalRotate(step.GetTransformValue(), step.Duration,
                step.UsingFastBeyond ? RotateMode.FastBeyond360 : RotateMode.Fast);

        private Tween CreateLocalMoveTween(TransformTweenStep step)
            => (IsCanvas ? _rectTrm : _transform).DOLocalMove(step.GetTransformValue(), step.Duration);

        private Tween CreateRotationTween(TransformTweenStep step)
            => (IsCanvas ? _rectTrm : _transform).DORotate(step.GetTransformValue(), step.Duration,
                step.UsingFastBeyond ? RotateMode.FastBeyond360 : RotateMode.Fast);

        private Tween CreateColorTween(TransformTweenStep step)
        {
            if (IsCanvas)
            {
                if (_graphic == null)
        ...
        private Tween CreateFadeTween(TransformTweenStep step)
        {
        ...
        private Tween CreateSizeDeltaTween(TransformTweenStep step)
        {
            if (_rectTrm == null)
        ...
        private Tween CreateFillAmountTween(TransformTweenStep step)
        {
            if (_image == null)
