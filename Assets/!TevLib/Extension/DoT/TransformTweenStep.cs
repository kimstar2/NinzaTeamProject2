...
{
    [Serializable]
    public struct TransformTweenStep
    {
        ...
        [field:SerializeField] public Transform MinTransformValue { get; private set; }
        [field:SerializeField] public Transform MaxTransformValue { get; private set; }
        
        public Vector3 GetTransformValue()
        {
            Vector3 resultVector = MinTransformValue.position;
            if (IsRandomizeValue)
            {
                float x = Random.Range(MinTransformValue.position.x, MaxTransformValue.position.x);
                float y = Random.Range(MinTransformValue.position.y, MaxTransformValue.position.y);
                float z = Random.Range(MinTransformValue.position.z, MaxTransformValue.position.z);
